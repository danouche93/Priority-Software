...
function parseSearchResponse(data: unknown): SearchResponse {
  if (typeof data !== 'object' || data === null) {
    throw new ApiError('Unexpected response shape from the search API.', 'unknown')
  }
  ...
  const previousCursor = typeof record.previousCursor === 'string' ? record.previousCursor : null
...
async function handleResponse(response: Response): Promise<SearchResponse> {
  if (!response.ok) {
    ...
    throw new ApiError(`Search request failed with status ${response.status}.`, kind)
  }
...
async function performFetch(url: URL, signal: AbortSignal | undefined): Promise<SearchResponse> {
  try {
    ...
    throw new ApiError('Network error while searching. Check your connection and try again.', 'network')
  }
}

export function searchByTerm({ query, limit = 6, signal }: SearchByTermParams): Promise<SearchResponse> {
...
export function searchByCursor({ cursor, signal }: SearchByCursorParams): Promise<SearchResponse> {
  const url = new URL(SEARCH_ENDPOINT, API_BASE_URL)
